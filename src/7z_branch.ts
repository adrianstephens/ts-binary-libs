...
function branchX86(data: Uint8Array, pc = 0) {
...
function branchPPC(data: Uint8Array, pc = 0) {
	const out = data.slice();
...
function branchARM(data: Uint8Array, pc = 0) {
	const out = data.slice();
...
function branchARMT(data: Uint8Array, pc = 0) {
	const out = data.slice();
...
function branchSPARC(data: Uint8Array, pc = 0) {
	const out = data.slice();
	const out4 = bin.typedArray.as(out, 'Uint32', true);
	const pc4   = pc >>> 2;
	for (let i = 0; i < out4.length; i ++) {
...
export function decodeBCJ2(inputs: Uint8Array[]) {
	...
	const appendUint32LE = (branch: number) => {
	...
	while (mainPos < main.length) {
		...
		if (rc.decodeBit(probs, ((((0 - c) & (value >>> 24)) + c + ((value >>> 5) & 1)) & 0x1ff) >>> 0) !== 0) {
			...
			appendUint32LE(target - (out.length + 4));
		}
	}
